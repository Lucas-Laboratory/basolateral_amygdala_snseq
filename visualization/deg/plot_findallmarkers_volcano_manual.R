...
# Generate volcano plots for specific clusters/genes using FindAllMarkers CSV input.
#
# Inputs
#   --deg-csv: FindAllMarkers CSV (required).
#   --output-pdf: destination PDF (required).
#   --clusters: comma-separated cluster IDs to plot (default: all).
#   --min-pct-diff: minimum |pct_diff| for high-significance (default: 0.1).
#   --min-p-adj: adjusted p-value cutoff (default: 0.01).
#   --min-logfc: minimum |log2FC| (default: 1.5).
...
#   --patterns-to-remove: optional gene exclusion regex (comma-separated).
...
main <- function() {
  option_list <- list(
    ...
    make_option("--min-logfc", type = "double", default = 1.5),
  ...
  parser <- OptionParser(option_list = option_list,
                         usage = "plot_findallmarkers_volcano_manual.R --deg-csv FILE --output-pdf FILE [options]")
  ...
  dev.off()
  message("Saved manual volcano plots to ", args$`output-pdf`)
}
