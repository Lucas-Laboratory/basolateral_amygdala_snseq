"""
This file includes code derived from: Yosef Lab, Weizmann Institute of Science
...
class BaseModelClass(metaclass=BaseModelMetaClass):
    ...
    def get_epoch_metrics(
        ...
        history: str | dict[str, pd.DataFrame] | None = None,
        add_kl_divergence: bool = True,
    ) -> pd.DataFrame:
        """Return a tidy DataFrame with per-epoch training metrics.

        Parameters
        ----------
        metrics
            Iterable of base metric names (without the ``_train``/``_validation`` suffix) to
            include. Defaults to commonly logged ELBO components.
        stages
            Training stages to include (e.g., ``\"train\"`` or ``\"validation\"``). If ``None``,
            looks for whichever stages are available in the history.
        history
            Source of metric history. ``None`` uses ``self.history_``. A string looks up the
            corresponding ``<string>_history_`` attribute on the model (useful for models like
            scANVI that track multiple phases). A mapping can be provided directly if you already
            have the history dictionary.
        add_kl_divergence
            If ``True`` and both ELBO and reconstruction loss are present, adds a derived
            ``kl_divergence`` column equal to ``elbo - reconstruction_loss``.

        Returns
        -------
        pandas.DataFrame
            DataFrame with columns ``epoch``, ``stage`` and the requested metrics, ready for
            plotting epoch-based curves.
        """
        history_source: dict[str, pd.DataFrame] | None
        if history is None:
            history_source = self.history_
        elif isinstance(history, str):
            attr_name = f"{history}_history_"
            if not hasattr(self, attr_name):
                raise AttributeError(
                    f"History source '{history}' not found on model as '{attr_name}'."
                )
            history_source = getattr(self, attr_name)
        else:
            history_source = history

        if history_source is None:
            raise RuntimeError("No training history is available. Train the model first.")
        if not isinstance(history_source, dict):
            raise TypeError("Training history is expected to be a dictionary of DataFrames.")

        default_metrics = ("elbo", "reconstruction_loss", "kl_local", "kl_global")
        metrics = list(dict.fromkeys(metrics or default_metrics))

        if stages is None:
            detected_stages = {
                key.rsplit("_", 1)[-1] for key in history_source if "_" in key
            }
            stage_order = ["train", "validation", "test"]
            stages = [s for s in stage_order if s in detected_stages]
            for stage_name in detected_stages:
        ...
        for stage in stages:
            ...
            for metric in metrics:
                ...
                metric_history = history_source.get(key)
                if metric_history is None:
                    continue
                renamed = metric_history.rename(columns={key: metric}).copy()
                stage_frames.append(renamed)
            if not stage_frames:
                continue
            stage_df = pd.concat(stage_frames, axis=1)
            stage_df = stage_df.reset_index()
            stage_df = stage_df.rename(columns={stage_df.columns[0]: "epoch"})
            stage_df["stage"] = stage
            frames.append(stage_df)

        if not frames:
            available = ", ".join(sorted(history_source.keys()))
            requested = ", ".join(metrics)
            raise ValueError(
                f"No matching metrics found. Requested metrics: {requested}. "
                f"Available keys: {available or 'none'}."
            )

        result = pd.concat(frames, ignore_index=True)

        if add_kl_divergence and {"elbo", "reconstruction_loss"}.issubset(result.columns):
            result["kl_divergence"] = result["elbo"] - result["reconstruction_loss"]

        preferred_order = ["epoch", "stage", "elbo", "reconstruction_loss", "kl_divergence"]
        preferred_order += [m for m in ("kl_local", "kl_global") if m in result.columns]
        for metric in metrics:
            if metric not in preferred_order and metric in result.columns:
                preferred_order.append(metric)

        ordered_columns = [col for col in preferred_order if col in result.columns]
        result = result[ordered_columns].sort_values(["stage", "epoch"]).reset_index(drop=True)
        return result
